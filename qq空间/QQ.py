...
def web_login_cookie():
    ...
    qq_account = '1612893772'
    qq_password = '13974162858x'
    login(driver,qq_account,qq_password)
    time.sleep(10)
    driver.get('https://user.qzone.qq.com/{}'.format(qq_account))
    cookie = ''
    for elem in driver.get_cookies(): # 获取相关的Cookie
        # elem 为 dict类型
...
def friend_db(dicts,name=''):
    ...
    friend_db_dict = {
        ...
        dicts['country'] + dicts['province'] + dicts['city.js'],dicts['cco'] + dicts['cp'] + dicts['cc'], dicts['cb']],
        'friendNet': [
        dicts['uin'], dicts['nickname'], dicts['spacename'], dicts['desc'], dicts['signature']]
    }
    conn = pymssql.connect(host='localhost', user='YUANWEI', password='1223456', database='friendDB',
                            charset='utf8')
    cur = conn.cursor()
